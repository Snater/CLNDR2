...
describe('pagination.scope set to `week`', () => {
	const oneWeekTemplate = `
		<div class="clndr-controls">
			<div class="clndr-previous-button" role="button">previous</div>
			<div class="week">Week <%= format(interval.start, 'w') %> in <%= format(interval.start, 'yyyy') %></div>
			<div class="clndr-next-button" role="button">next</div>
		</div>
		<div class="clndr-grid">
			<% items.forEach(day => { %>
				<div class="<%= day.classes %>" role="button"><%= format(day.date, 'd') %></div>
			<% }); %>
		</div>
		<div class="clndr-today-button" role="button">Current week</div>
	`;
	...
	test('Click on a day while the identifier class is unexpectedly not assigned', async () => {
		...
		const weekElement = screen.getByText('18');
		expect(weekElement instanceof Element).toBeTruthy();
		(weekElement as Element).classList.remove('calendar-day-2024-01-18');
		await user.click(weekElement as Element);
