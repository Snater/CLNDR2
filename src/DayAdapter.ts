...
export class DayAdapter extends Adapter {
	...
	initInterval(pageSize: number, startOn: Date | undefined, weekOffset: number): Interval {
		const start = startOfDay(startOn ? startOn : setDay(new Date(), weekOffset));
		const end = endOfDay(addDays(start, pageSize - 1));

		return [start, end];
	}

	initStartConstraint(constraintStart: Date, interval: Interval, pageSize: number): Interval {
		...
		adjustedInterval[1] = endOfDay(addDays(adjustedInterval[0], pageSize - 1));

		return adjustedInterval;
	}

	initEndConstraint(constraintEnd: Date, interval: Interval, pageSize: number): Interval {
		const adjustedInterval: Interval = [interval[0], interval[1]];

		if (isAfter(adjustedInterval[0], addWeeks(constraintEnd, 1))) {
			adjustedInterval[0] = startOfDay(
				subDays(endOfWeek(constraintEnd), pageSize - 1)
			);
	...
	setDay(day: Date, pageSize: number, startOn?: Date): Interval {
		...
		return [start, endOfDay(addDays(start, pageSize - 1))];
	}
